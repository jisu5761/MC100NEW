...
//#include "radio_config_Si4464_235tx.h"
//#include "radio_config_Si4464_235rx.h"
...
#ifdef ARDUINO
#define	 PIN_nSEL358	12
#define	 PIN_SDN358		14
#define	 PIN_SDI		33
#define	 PIN_SCK		25
#define	 PIN_SDO		32
...
#define spiSelect()				(digitalWrite(PIN_nSEL358, LOW))
#define spiDeselect()			(digitalWrite(PIN_nSEL358, HIGH))

#define	PIN_SDI_HIGH			(digitalWrite(PIN_SDI, HIGH))
#define	PIN_SDI_LOW				(digitalWrite(PIN_SDI, LOW))

#define	PIN_SCK_HIGH			(digitalWrite(PIN_SCK, HIGH))
#define	PIN_SCK_LOW				(digitalWrite(PIN_SCK, LOW))

#define	PIN_SDO_IN				digitalRead(PIN_SDO)
//#define spi_transfer_nr(data)	(SPI.transfer(data))
#define spi_transfer_nr(data)	(spi_transfer(data))
...
#define spi235Select()			(CSN_PORT &= ~_BV(CSN_BIT))
#define spi235Deselect()		(CSN_PORT |= _BV(CSN_BIT))
#define spi358Select()			digitalWrite(SI446X_358CSN,HIGH)
#define spi358Deselect()		digitalWrite(SI446X_358CSN,LOW)
#endif

static const uint8_t config[] PROGMEM = RADIO_CONFIGURATION_DATA_ARRAY;
//static const uint8_t config235tx[] PROGMEM = TX235RADIO_CONFIGURATION_DATA_ARRAY;
//static const uint8_t config235rx[] PROGMEM = RX235RADIO_CONFIGURATION_DATA_ARRAY;
...
//uint8_t spi_transfer_nr(uint8_t data);

//#define	spi_transfer(data) spi_transfer_nr(data)


uint8_t	spi_transfer(uint8_t	dat)
{
	uint8_t i, dummy, rts = 0;
	PIN_SDI_LOW;
	PIN_SCK_LOW;
	dummy = dat;
	for (i = 0; i < 8; i++)
...
{
#ifdef ARDUINO
	digitalWrite(PIN_SDN358, HIGH);
	delay_ms(50);
	digitalWrite(PIN_SDN358, LOW);
	delay_ms(50);
#else
...
static void resetDevice235(void)
{
#ifdef ARDUINO
	//digitalwrite(pin_sdn235, high);
	//delay_ms(50);
	//digitalwrite(pin_sdn235, low);
	//delay_ms(50);
#else
	SDN_PORT |= _BV(SDN_BIT);
	delay_ms(50);
...
static void applyStartupConfig235tx(void)
{
	uint8_t buff[17];
	//for (uint16_t i = 0;i<sizeof(config235tx);i++)
	//{
	//	memcpy_P(buff, &config235tx[i], sizeof(buff));
	//	doAPI(&buff[1], buff[0], NULL, 0);
	//	i += buff[0];
	//}
}

static void applyStartupConfig235rx(void)
{
	uint8_t buff[17];
	//for (uint16_t i = 0;i<sizeof(config235rx);i++)
	//{
	//	memcpy_P(buff, &config235rx[i], sizeof(buff));
	//	doAPI(&buff[1], buff[0], NULL, 0);
	//	i += buff[0];
	//}
}
